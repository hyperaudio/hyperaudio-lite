...
| `scrollContainer` | _transcript element_ | The element that scrolls to follow playback — an element or an element id. Pass `document.scrollingElement` for layouts where the page itself scrolls rather than the transcript |
...
Pressing <kbd>Enter</kbd> or <kbd>Space</kbd> on a focused transcript word sets the playhead, exactly like a click. Words are `<span>`s, so they aren't focusable by default — add `tabindex="0"` to the word elements in your transcript markup to enable keyboard navigation:
