...
class HyperaudioLite {
  ...
  init = (mediaElementId, m, a, d, w) => {
    ...
    if (hashVar === this.transcript.id) {
      this.hashArray = windowHash
        .substr(this.transcript.id.length + 2)
        .split(",");
    } else {
      this.hashArray = [];
    }
  ...
  getSelectionMediaFragment = () => {
    ...
    if (selection.toString() !== "") {
      ...
      if (
        aNode.getAttribute("data-m") === null ||
        aNode.className === "speaker"
      ) {
        aNode = aNode.nextElementSibling;
      }

      if (
        fNode.getAttribute("data-m") === null ||
        fNode.className === "speaker"
      ) {
      ...
      let nodeDuration =
        Math.round((fNodeTime + fNodeDuration - aNodeTime) * 10) / 10;

      if (aNodeTime >= fNodeTime) {
        nodeStart = fNodeTime;
        nodeDuration =
          Math.round((aNodeTime + aNodeDuration - fNodeTime) * 10) / 10;
      }
      ...
      fragment =
        ...
        nodeStart +
        "," +
        Math.round((nodeStart + nodeDuration) * 10) / 10;
    }
  ...
  checkPlayHead = () => {
    ...
    if (this.end && this.end < this.currentTime) {
      ...
      if (index > 0) {
        ...
        if (currentParaIndex != this.paraIndex) {
          if (
            ...
            if (scrollNode !== null) {
              if (
                typeof this.scrollerContainer !== "undefined" &&
      ...
      if (this.wordArr[index]) {
        interval = parseInt(
          this.wordArr[index].n.getAttribute("data-m") - this.currentTime * 1000
        );
      }
    ...
    if (this.webMonetization === true) {
      ...
      let paymentPointer = this.checkPaymentPointer(
